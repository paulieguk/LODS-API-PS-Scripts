<#
    ...
    The initial use of this script would allow aspect of the exams quality to be reviewed.  Examples of this would be:
        ...
        The script does not require all the Lab Instances specficied are for the same exam.  If the Lab Instances are for different exams the list is not grouped based on Exam name.
    ...
    The script will now return a warning if the supplied Lab Instances do not contain any exams.
    The following ErrorLevels are retuned by the script:
        0 = Successful / No Error
        1 = Lab Instances supplied to not contain any exam data (IsExam is not set to True)
...
For ($i = 0; $i -lt $p.Count; $i++) {
    ...
    if ($apiResponse.IsExam -eq $false) {
    }
    Else {
...
## Check to confirm activity data has been generated
if ( $CSVOutput.Length -eq 0 ) {
    Write-Warning -Message "Lab Instances do not contain any activity data" 
    Exit 1
}
